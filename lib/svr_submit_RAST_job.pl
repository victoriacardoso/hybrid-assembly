...
my $usage = $self  . qq(  --user UserName  --passwd Passwd  (--genbank GenbankFilename | --fasta FASTAfilename) --domain (Bacteria|Archaea)  [--taxon_ID NCBI_taxonomy_ID(def:666666)]  [--bioname "genus species strain" (def: "Unknown sp.")]  [--genetic_code (11|4)] [--gene_caller (rast|glimmer3)] [--determine_family] [--reannotate_only] [--test] [--nonActive] [--kmerDataset version]);
...
{
    $arg_hashP->{-kmerDataset} = $kmer_dataset;
}
