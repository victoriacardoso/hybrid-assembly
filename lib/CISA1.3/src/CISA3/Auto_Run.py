...
makeblastdb=sys.argv[4]
blastn=sys.argv[5]
my_thr=float(sys.argv[6])
d=dict() #big dict()
...
def run_Blast():
    ...
    print "Done!"
def make_new_result():   # Remove map which align_len/contig_len < 0.3
    ...
    while True:
        ...
        for i in range(hits): # sometimes, hits number isn't exact
            ...
            if (align_len/q_len)<0.3 and (align_len/s_len)<0.3 and align_len < (my_thr*0.3):continue
            hits_list.append(s)
